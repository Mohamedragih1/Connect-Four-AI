...
def minimax(board, depth, maximizing_player, max_depth, alpha, beta):
    ...
    if maximizing_player:
        ...
        for col in range(COLS):
            if is_valid_move(board, col):
                ...
                value = max(value, minimax(temp_board, depth-1, False, max_depth, alpha, beta))
        ...
        for col in range(COLS):
            if is_valid_move(board, col):
                ...
                value = min(value, minimax(temp_board, depth-1, True, max_depth, alpha, beta))
                beta = min(beta, value)
                if beta <= alpha:
                    break  # Alpha cut-off
        return value
...
def get_best_move(board, depth):
    ...
    for col in range(COLS):
        if is_valid_move(board, col):
            ...
            score = minimax(temp_board, depth - 1, False, depth, -INF, INF)  # Pass depth limit
